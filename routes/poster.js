...
router.post('/', asyncHandler(async (req, res) => {
    try {
        uploadPosters.single('img')(req, res, async function (err) {
            ...
            let imageUrl = 'no_url';
            if (req.file) {
                imageUrl = `http://192.168.0.203:3000/image/poster/${req.file.filename}`;
            }
            ...
            try {
                ...
                io.emit('posterUpdate', { action: 'added', data: newPoster });
...
router.put('/:id', asyncHandler(async (req, res) => {
    try {
        ...
        uploadPosters.single('img')(req, res, async function (err) {
            ...
            if (req.file) {
                image = `http://192.168.0.203:3000/image/poster/${req.file.filename}`;
            }
            ...
            try {
                ...
                io.emit('posterUpdate', { action: 'updated', data: updatedPoster });
...
router.delete('/:id', asyncHandler(async (req, res) => {
    ...
    try {
        ...
        io.emit('posterUpdate', { action: 'deleted', data: { posterID } });
